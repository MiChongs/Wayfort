...
var groups = []Group{
	...
	{ID: "geoip", Title: "地理位置与异常登录", Icon: "globe", Order: 15,
		Subtitle: "IP 地理库（含自动更新）与登录风险评分、暴力破解告警"},
...
var specs = []Spec{
	...
	// 异常登录检测策略归入「地理位置与异常登录」组，与地理库放在一起；运行时热生效。
	{Key: "auth.anomaly.enabled", Group: "geoip", Type: TypeBool, Label: "异常登录检测", Live: true,
		Help: "对新 IP / 新设备 / 异地 / 新运营商 / 不可能位移等行为打分并标记异常登录。"},
	{Key: "auth.anomaly.notify_email", Group: "geoip", Type: TypeBool, Label: "异常登录邮件提醒", Live: true,
		DependsOn: "auth.anomaly.enabled", DependsValue: "true",
		Help: "识别到异常登录时向账号邮箱发送提醒（同时始终写入站内通知中心）。"},
	{Key: "auth.anomaly.notify_admins", Group: "geoip", Type: TypeBool, Label: "通知安全管理员", Live: true,
		DependsOn: "auth.anomaly.enabled", DependsValue: "true",
		Help: "异常登录与暴力破解告警同时通知安全团队（持有 security:manage / audit:read / system:admin 的人）。"},
	{Key: "auth.anomaly.score_threshold", Group: "geoip", Type: TypeInt, Label: "异常判定阈值", Unit: "分", Live: true,
		Min: f(1), Max: f(100), DependsOn: "auth.anomaly.enabled", DependsValue: "true",
		Help: "风险评分（0–100）达到此值即判定为异常登录；不可能位移始终判异常。"},
	{Key: "auth.anomaly.history_window", Group: "geoip", Type: TypeInt, Label: "历史比对条数", Unit: "条", Live: true, Advanced: true,
		Min: f(5), Max: f(200), DependsOn: "auth.anomaly.enabled", DependsValue: "true",
		Help: "与最近多少次成功登录比对 IP / 国家 / 设备 / 运营商。"},
	{Key: "auth.anomaly.impossible_travel_kmh", Group: "geoip", Type: TypeFloat, Label: "不可能位移速度", Unit: "km/h", Live: true, Advanced: true,
		Min: f(0), Max: f(5000), DependsOn: "auth.anomaly.enabled", DependsValue: "true",
		Help: "两次登录间隐含地速超过此值即判为不可能位移（疑似异地）；0 关闭该规则。"},
	{Key: "auth.anomaly.brute_force_threshold", Group: "geoip", Type: TypeInt, Label: "暴力破解阈值", Unit: "次", Live: true,
		Min: f(2), Max: f(1000), DependsOn: "auth.anomaly.enabled", DependsValue: "true",
		Help: "窗口内同账号 / 同 IP 登录失败达到此次数即触发暴力破解告警。"},
	{Key: "auth.anomaly.brute_force_window", Group: "geoip", Type: TypeDuration, Label: "暴力破解统计窗口", Live: true, Advanced: true,
		DependsOn: "auth.anomaly.enabled", DependsValue: "true",
		Help: "统计登录失败次数的滚动时间窗，也是同一告警的去抖周期。"},

	// ---------------- 地理位置库 (request-time / updater → live) ----------------
	{Key: "geoip.enabled", Group: "geoip", Type: TypeBool, Label: "启用 IP 地理解析", Live: true,
		Help: "把登录来源 IP 解析为国家 / 地区 / 城市 / 运营商，写入登录历史并供异常检测使用。"},
	{Key: "geoip.auto_update", Group: "geoip", Type: TypeBool, Label: "自动更新地理库", Live: true,
		DependsOn: "geoip.enabled", DependsValue: "true",
		Help: "按更新周期自动下载并热替换地理库文件；关闭则只用本地已有文件（可手动「立即更新」）。"},
	{Key: "geoip.update_url", Group: "geoip", Type: TypeString, Label: "城市库下载地址", Live: true, Advanced: true,
		DependsOn: "geoip.enabled", DependsValue: "true",
		Placeholder: "https://.../dbip-city-lite-{year}-{month}.mmdb.gz",
		Help:        "城市/国家库下载地址，支持 .mmdb / .gz / .tar.gz；{year}/{month} 在下载时按当前 UTC 日期替换。"},
	{Key: "geoip.asn_update_url", Group: "geoip", Type: TypeString, Label: "ASN 库下载地址", Live: true, Advanced: true,
		DependsOn: "geoip.enabled", DependsValue: "true",
		Placeholder: "https://.../dbip-asn-lite-{year}-{month}.mmdb.gz",
		Help:        "可选的 ASN（运营商）库下载地址，留空则不解析运营商。"},
	{Key: "geoip.db_path", Group: "geoip", Type: TypeString, Label: "城市库文件路径", Live: true, Advanced: true,
		DependsOn: "geoip.enabled", DependsValue: "true",
		Help: "本地城市库 .mmdb 的存放路径，自动更新会原子替换此文件。"},
	{Key: "geoip.asn_db_path", Group: "geoip", Type: TypeString, Label: "ASN 库文件路径", Live: true, Advanced: true,
		DependsOn: "geoip.enabled", DependsValue: "true",
		Help: "本地 ASN 库 .mmdb 的存放路径。"},
	{Key: "geoip.update_interval", Group: "geoip", Type: TypeDuration, Label: "更新周期", Live: true,
		DependsOn: "geoip.auto_update", DependsValue: "true",
		Help: "地理库的过期阈值与检查周期（实际检查频率上限为每天一次）。"},
	{Key: "geoip.download_timeout", Group: "geoip", Type: TypeDuration, Label: "下载超时", Live: true, Advanced: true,
		DependsOn: "geoip.auto_update", DependsValue: "true",
		Help: "单次地理库下载的超时时间。"},
	{Key: "geoip.language", Group: "geoip", Type: TypeString, Label: "地名语言", Live: true, Advanced: true,
		DependsOn: "geoip.enabled", DependsValue: "true",
		Placeholder: "zh-CN", Help: "地名优先使用的语言（如 zh-CN / en），缺失时回退到英文。"},
	{Key: "geoip.allow_private_url", Group: "geoip", Type: TypeBool, Label: "允许内网下载源", Live: true, Advanced: true,
		DependsOn: "geoip.enabled", DependsValue: "true",
		Help: "默认禁止从内网/环回地址下载地理库（防 SSRF）。仅当使用内网镜像时开启。"},
...
var wiredLive = map[string]bool{
	// Anomaly detector + GeoIP service both expose ApplyConfig and are subscribed
	// to the center in main.go, so their tuning applies without a restart.
	"auth.anomaly.enabled":               true,
	"auth.anomaly.notify_email":          true,
	"auth.anomaly.notify_admins":         true,
	"auth.anomaly.score_threshold":       true,
	"auth.anomaly.history_window":        true,
	"auth.anomaly.impossible_travel_kmh": true,
	"auth.anomaly.brute_force_threshold": true,
	"auth.anomaly.brute_force_window":    true,
	"geoip.enabled":                      true,
	"geoip.auto_update":                  true,
	"geoip.update_url":                   true,
	"geoip.asn_update_url":               true,
	"geoip.db_path":                      true,
	"geoip.asn_db_path":                  true,
	"geoip.update_interval":              true,
	"geoip.download_timeout":             true,
	"geoip.language":                     true,
	"geoip.allow_private_url":            true,
