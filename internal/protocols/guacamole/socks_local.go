// Package guacamole bridges browser WebSocket sessions to a guacd daemon. The
// SOCKS5 listener here is the trick that lets guacd reach targets that sit
// behind our existing ContextDialer chain (SOCKS5 + SSH bastion). We expose a
// minimal CONNECT-only SOCKS5 server on 127.0.0.1; guacd is told via its
// socks-proxy-* parameters to dial through it, and every CONNECT we receive is
// translated into a chain.Build().DialContext() call.
package guacamole
...
//
// The listener is single-use: it is meant to back exactly one guacd session
// and accepts an unbounded number of TCP connects but always tunnels them to
// the same target. Close stops the acceptor goroutine.
...
// Useful as a sanity check so only the colocated guacd talks to us.
...
func (l *Listener) handle(ctx context.Context, c net.Conn) {
	...
	// We honour the *destination* the client asked for. In practice guacd will
	// dial the target we configured in the connect instruction, but treating
	// this as a generic SOCKS5 (rather than hard-wiring the target) keeps the
	// listener composable with other clients.
