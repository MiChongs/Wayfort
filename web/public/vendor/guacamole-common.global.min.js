...
Guacamole.Display = function() {
    ...
    this.drawBlob = function(layer, x, y, blob) {
        ...
        if (window.createImageBitmap) {
            ...
            // Load image from provided blob. If native decode rejects, use the
            // app-provided JS decoder hook so the render queue never wedges.
            window.createImageBitmap(blob).then(function bitmapLoaded(decoded) {
                bitmap = decoded;
                task.unblock();
            }).catch(function bitmapNativeDecodeFailed() {
                if (!window.__guacDecodeImage) {
                    task.unblock();
                    return;
                }
                window.__guacDecodeImage(blob).then(function bitmapFallbackLoaded(decoded) {
                    bitmap = decoded;
                    task.unblock();
                }, task.unblock);
            });
    ...
    this.draw = function(layer, x, y, url) {
        ...
        var task = scheduleTask(function __display_draw() {
            ...
            var drawable = decodedImage || image;

            // Draw the image only if it loaded without errors
            if (drawable && drawable.width && drawable.height)
                layer.drawImage(x, y, drawable);

        }, true);

        var image = new Image();
        image.onload = task.unblock;
        image.onerror = function imageNativeDecodeFailed() {
            if (!window.__guacDecodeImage || typeof url !== "string" || url.indexOf("data:image/") !== 0) {
                task.unblock();
                return;
            }
            window.__guacDecodeImage(url).then(function imageFallbackLoaded(decoded) {
