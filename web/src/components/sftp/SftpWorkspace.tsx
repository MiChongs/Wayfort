...
export type SftpWorkspaceProps = {
  nodeId: number
  // Whether to render the in-component NodeHeader. The standalone page
  // wants it (it's the only chrome around the SFTP body); the workspace
  // tab variant doesn't (the tab title already names the host).
  showNodeHeader?: boolean
  // Override the container's height. Standalone page uses
  // `calc(100vh - 3.5rem)`; embedded in a tab the parent already constrains
  // the height with flex, so callers can pass `h-full`.
  className?: string
}
